...
class Model:
	...
	def __init__(self):
		...
		fc1_input_size = ((image_dimension // (2**no_of_pooling_layer)) * (image_dimension // (2**no_of_pooling_layer))*patch_depth4)

		#initialize the weight and bias variables
		self.wb_variables = {

			#w1-w13 : convolutional layer's weight
			'w1'  : self.weight_variable_convolution(conv_weight_filter_size, image_depth, patch_depth1, 'w1'),

			'w2'  : self.weight_variable_convolution(conv_weight_filter_size, patch_depth1, patch_depth1, 'w2'),
		...
		self.x = tf.placeholder('float', [None, image_dimension, image_dimension, image_depth])
		...
		with tf.name_scope('Conv-group1-maxPool'):
			...
			first_pooling_layer = self.max_pooling(layer2_conv_w_relu, 2, 2, 'pooling-1')

		#second group of cnn
		with tf.name_scope('Conv-group2'):

			layer3_conv_w_relu = self.convolution_layer(first_pooling_layer, self.wb_variables['w3'], self.wb_variables['b3'], 1, 'conv1-group2', 'conv1-group2_actv')
		...
		with tf.name_scope('Conv-group2-maxPool'):
			...
			second_pooling_layer = self.max_pooling(layer4_conv_w_relu, 2, 2, 'pooling-2')
		...
		with tf.name_scope('Flatten-layer'):
			...
			self.reshape_output = tf.reshape(fifth_pooling_layer, [-1, output_shape])
